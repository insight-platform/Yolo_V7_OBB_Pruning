...
class MetaPruner():
    ...
    def __init__(
        ...
        self.DG_ref = dependency.DependencyGraph().build_dependency(
            model_ref,
            example_inputs=example_inputs,
            output_transform=output_transform,
            unwrapped_parameters=unwrapped_parameters,
    ...
    def get_all_groups(self):
        visited_layers = []
        for m, m_ref in zip(self.DG.module2node.keys(), self.DG_ref.module2node.keys()):
            ...
            group_ref = self.DG_ref.get_pruning_group(m_ref, pruner.prune_out_channels, list(range(layer_channels)))
            prunable_group = True
            for (dep, _), (dep_ref, _) in zip(group, group_ref):
    ...
    def prune_local(self):
        ...
        for group, group_ref in self.get_all_groups():
            # check pruning rate
            if self._check_sparsity(group):
                module = group[0][0].target.module
                module_ref = group_ref[0][0].target.module
                ...
                group_ref = self.DG_ref.get_pruning_group(
                    module_ref, pruning_fn, pruning_idxs.tolist())
