...
from yolov7obb.utils.general import xywh2xyxy, xyxy2xywh
from yolov7obb.utils.metrics import fitness
from yolov7obb.utils.rboxs_utils import poly2hbb, poly2rbox, rbox2poly
...
def plot_images_debug(image, annotations, path=None, fname='images.jpg', names=None):
    """
    Args:
        image (np.array): (height, width, 3)
        annotations (np.array[clsid x1 y1 x2 y2 x3 y3 x4 y4]):
        path (Path): (b)
        fname (str): (1)
        names :
    ...
    if path is None:
        path = Path('.')
    if np.max(image_tmp) <= 1:
        image_tmp *= 255  # de-normalise (optional)
    h, w, _ = image_tmp.shape  # batch size, _, height, width
    ns = 1

    # Annotate
    fs = int((h + w) * ns * 0.01)  # font size
    annotator = Annotator(np.ascontiguousarray(image_tmp), line_width=round(fs / 10), font_size=fs, pil=True)
    for j, annotation in enumerate(annotations.tolist()):
        cls = annotation[0]
        color = colors(cls)
        cls = names[cls] if names else cls
        label = f'{cls}'
        annotator.poly_label(annotation[1:], label, color=color)
    annotator.im.save(str(path / fname))  # save
