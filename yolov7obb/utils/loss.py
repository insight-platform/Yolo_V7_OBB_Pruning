...
from yolov7obb.utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy
from yolov7obb.utils.torch_utils import is_parallel


class IComputeLoss(ABC):
    @abstractmethod
...
class ComputeLossOBB(IComputeLoss):
    """Compute losses for an object detection model.
    This class extends the IComputeLoss interface and implements the loss computation for object detection tasks.
    It calculates the loss based on binary cross entropy with logits and focal loss,
    as well as hyperparameters for the model.
    """
    def __init__(
            self,
            model: nn.Module,
        ...
        """Initialize the ComputeLossOBB object.

        Args:
            model (nn.Module): Object detection model.
            autobalance (bool, optional): Flag for autobalance. Defaults to False.
            nc (int, optional): Number of classes. Defaults to 80.

        """
        super(ComputeLossOBB, self).__init__()
        self.nc = nc
        ...
        bce_theta = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['theta_pw']], device=device))
        ...
        if g > 0:
            ...
            bce_obj =FocalLoss(bce_obj, g)
            bce_theta = FocalLoss(bce_theta, g)
